...
| `pathway_b_traversal.py` | Deterministic page-frame walk for composition files. Bounded depth (default 20) + per-page frame cap. Emits `composition_tree` + `remote:true` reference list for Lane 6 consumption. Page fetches use **(2,5,12,30)s ×4 429 backoff** + **adaptive inter-page pacing** (0.5s between fetches on files >20 pages only — spreads Figma API load; Helix-17 unaffected) — strengthened from (1,3,8)×3 after DGX (36 pages) 429'd 32/36. Exhausted retries surface fail-loud as `malformed_node_data`, never silent. |
...
| `http_errors.py` | **Sustained-429 guard** (workflow-level, v3.1) + **403 classifier**. `with_account_level_retry` wraps an extraction: bare `/files/{key}?depth=1` preflight-probe detects a *sustained* throttle (distinct from per-request 429s the lanes already handle), waits a bounded deterministic `30→60→120s` re-probing between, emitting `sustained_429_retry_heartbeat` events (~40s cadence, INFO) throughout, and on persistence returns a fail-loud escalation (`error_class: account_level_rate_limit`) with `throttle_scope` (`per_file`\|`pat_wide`\|`undetermined`) + `undetermined_reason` (`reference_env_unset`\|`reference_probe_error`\|`cross_probe_cap_exceeded`) from a cross-probe of a configured reference file (env `FIGMA_RATE_LIMIT_REFERENCE_FILE`; unset → loud startup warning + scope disabled), a global cross-probe cap (FM-1, env `FIGMA_RATE_LIMIT_CROSS_PROBE_MAX_PER_HOUR`, default 10/h), `retry_after` hint, `heartbeats_emitted`, and a `metrics` block. Also `classify_http_error`/`raise_for_figma_status` sub-classify a **403 body**: `file_export_disabled` (the "File not exportable" content-protection lock — B-S), `enterprise_scope` ("Invalid scope"), else `forbidden`; wired into the REST body-hitting lanes (Lane 3/5/7 + Pathway B), Lane 2 metadata endpoints stay accessible under the lock. No partial results. Per-lane backoff untouched. |
