...
| `http_errors.py` | **Sustained-429 guard** (workflow-level). `with_account_level_retry` wraps an extraction: bare `/files/{key}?depth=1` preflight-probe detects a *sustained* throttle (distinct from per-request 429s the lanes already handle), waits a bounded deterministic `30→60→120s` re-probing between, and on persistence returns a fail-loud escalation (`error_class: account_level_rate_limit`) with a `throttle_scope` (`per_file`\|`pat_wide`\|`undetermined`) from a cross-probe of a configured known-good reference file (env `FIGMA_RATE_LIMIT_REFERENCE_FILE`; unset → `undetermined`) + `retry_after` hint. No partial results. Per-lane backoff untouched. |
